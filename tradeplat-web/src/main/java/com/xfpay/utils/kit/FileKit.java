...
public abstract class FileKit {
	 ...
	 *             如果路径非法
	 */
	public static String normalizeAbsolutePath(String path) throws RuntimeException {
	 ...
	 *             如果路径非法
	 */
	public static String normalizeAbsolutePath(String path, boolean removeTrailingSlash) throws RuntimeException {
	 ...
	 * 
	 * <ol>
	 * <li>路径为null，则返回null。</li>
	 * <li>将所有backslash("\\")转化成slash("/")。</li>
	 * <li>去除重复的"/"或"\\"。</li>
	 * <li>去除"."，如果发现".."，则向上朔一级目录。</li>
	 * <li>空绝对路径返回"/"，空相对路径返回"./"。</li>
	 * <li>保留路径末尾的"/"（如果有的话）。</li>
	 * <li>对于绝对路径，如果".."上朔的路径超过了根目录，则看作非法路径，返回null。</li>
	 ...
	 *             如果路径非法
	 */
	public static String normalizeRelativePath(String path) throws RuntimeException {
	 ...
	 *             如果路径非法
	 */
	public static String normalizeRelativePath(String path, boolean removeTrailingSlash) throws RuntimeException {
	 ...
	 *             如果路径非法
	 */
	public static String normalizePath(String path) throws RuntimeException {
	 ...
	 *             如果路径非法
	 */
	public static String normalizePath(String path, boolean removeTrailingSlash) throws RuntimeException {
	...
	private static String normalizePath(String path, boolean forceAbsolute, boolean forceRelative,
			boolean removeTrailingSlash) throws RuntimeException {
	 ...
	 *             如果路径非法
	 */
	public static String getAbsolutePathBasedOn(String basedir, String path) throws RuntimeException {
		// 如果path为绝对路径，则规格化后返回
	...
	public static String getSystemDependentAbsolutePathBasedOn(String basedir, String path) {
		...
		if (pathFile.isAbsolute()) {
			...
			if (baseFile.isAbsolute()) {
				...
				throw new RuntimeException("Basedir is not absolute path: " + basedir);
			}
		}
	 ...
	 *             如果路径非法
	 */
	public static String getRelativePath(String basedir, String path) throws RuntimeException {
	 ...
	 *            要检查的路径
	 * @param isWindows
	 *            是否是windows路径，如果为true，则支持驱动器名和UNC路径
	 * 
	 * @return 规格化的路径，如果path非法，或basedir为 null，则返回null
	 */
	private static String getPathBasedOn(String basedir, String path, boolean isWindows) {
		...
		// 如果是绝对路径，则直接返回
		if ((prefix.length() > 0)
				|| ((path.length() > prefix.length()) && ((path.charAt(prefix.length())+"").equals(SLASH_CHAR)))) {
		...
		if ((basedir.length() > 0) && (path.length() > 0) && ((basedir.charAt(basedir.length() - 1)+"").equals(SLASH_CHAR))) {
	...
	private static String getRelativePath(String basedir, String path, boolean isWindows) {
		...
		// 按"/"分隔basedir和path
		String[] baseParts = basedir.substring(basePrefix.length()).split(SLASH_CHAR);
		String[] parts = path.substring(prefix.length()).split(SLASH_CHAR);
		StringBuilder builder = new StringBuilder();
		int i = 0;
